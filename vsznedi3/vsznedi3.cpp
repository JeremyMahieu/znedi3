...
public:
	...
	void init(const ConstMap &in, const Map &out, const Core &core) override
	{
		...
		if (m_vi.format.colorFamily == cfYUV) {
			m_nnedi3_chroma.reset(znedi3_filter_create(nnedi3_weights.get(), &nnedi3_params));
			if (!m_nnedi3_chroma)
				throw std::runtime_error{ "failed to create nnedi3" };
		}
	...
	ConstFrame get_frame(int n, const Core &core, const FrameContext &frame_context, void *) override
	{
		...
		size_t tmp_size = znedi3_filter_get_tmp_size(m_nnedi3.get(), m_vi.width, m_vi.height / 2);
		if (m_nnedi3_chroma)
			tmp_size = std::max(tmp_size, znedi3_filter_get_tmp_size(m_nnedi3_chroma.get(), m_vi.width, m_vi.height / 2));
		...
		for (int p = 0; p < src_frame.video_format().numPlanes; ++p) {
			...
			znedi3_filter_process(nnedi3, width, height, src_field_p, src_field_stride, dst_field_p, dst_field_stride, tmp, !src_parity);

...
znedi3_filter *znedi3_filter_create(const znedi3_weights *weights, const znedi3_filter_params *params, unsigned width, unsigned height) try
{
	...
	std::unique_ptr<znedi3::znedi3_filter2> filter = std::make_unique<znedi3::znedi3_filter2>(*nnedi3, *params, width, height);
...
{
	delete static_cast<znedi3::znedi3_filter2 *>(ptr);
}

size_t znedi3_filter_get_tmp_size(const znedi3_filter *ptr)
{
	return static_cast<const znedi3::znedi3_filter2 *>(ptr)->get_tmp_size();
}
...
{
	static_cast<const znedi3::znedi3_filter2 *>(ptr)->process(src, src_stride, dst, dst_stride, tmp, parity);
}
